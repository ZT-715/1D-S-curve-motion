...
float* calculate_time_1d(float time[6],
    ...
    if (2*velocity[0]*time[0] > displacement/2) {
    ...
    if (2*velocity[0]*time[0] + time[1]*(acceleration_max*time[1]/2 + velocity[0]) > displacement/2) {
        time[1] = ( - (3*jerk_max*time[0]*time[0])
                    + sqrtf(jerk_max*time[0]*time[0]*time[0]*time[0] + 4*jerk_max*time[0]*displacement)
                  ) / (2*jerk_max*time[0]);
    }
    ...
    time[3] =  (displacement - 2*(2*velocity[1]*time[0] + time[1]*(acceleration_max*time[1]/2 + velocity[0]))
               ) / velocity[2];
    time[4] = time[0];
    time[5] = time[1];
    time[6] = time[2];
...
void generate_curve_1d(float time[6],
    ...
    int const steps1 = (int) (time[0]/time_delta);
    ...
    if (steps7 != step_counter) {
        printf("Error in generate_curve_1d: step total not equal steps counted. (%d != %d) \n", steps7, step_counter);
    }
